...
async function dlImage(fmt){
  const html=lbHTML||curHTML;
  if(!html)return;
  loading(true,'Gerando imagem... aguarde uns segundos.');

  // Cria um container oculto e injeta o HTML do flyer diretamente
  const wrap=document.createElement('div');
  wrap.style.cssText='position:fixed;left:-9999px;top:0;width:1080px;height:1080px;overflow:hidden;z-index:1';
  // Extrai apenas o conteudo do body do flyer para injetar
  const parser=new DOMParser();
  const doc=parser.parseFromString(html,'text/html');
  // Copia os estilos do flyer para o wrap
  ...
  // Copia links de fonte
  const links=doc.querySelectorAll('link');
  links.forEach(l=>{
    const lk=l.cloneNode(true);
    document.head.appendChild(lk);
  });
  // Copia o body
  const bodyClone=document.createElement('div');
  bodyClone.style.cssText='width:1080px;height:1080px;overflow:hidden;position:relative';
  ...
  try{
    let dataUrl;
    if(fmt==='png'){
      dataUrl=await htmlToImage.toPng(bodyClone,{
      ...
      dataUrl=await htmlToImage.toJpeg(bodyClone,{
        width:1080,height:1080,
        quality:0.95,
        pixelRatio:1,
        skipAutoScale:true,
    ...
    a.href=dataUrl;
    a.download='speedflyer_'+Date.now()+'.'+fmt;
    a.click();
    toast('Flyer baixado em '+fmt.toUpperCase()+'! Pronto para WhatsApp e Instagram 🎉');
  }catch(e){
    console.error('html-to-image error:',e);
    // Fallback confiavel: abre numa nova aba para salvar
    const blob=new Blob([html],{type:'text/html'});
    const url=URL.createObjectURL(blob);
    const w=window.open(url,'_blank');
    toast('Abriu em nova aba. Pressione Ctrl+P → Salvar como PDF para ter a imagem.');
  }finally{
