...
async function dlImage(fmt){
  ...
  loading(true,'Gerando imagem... aguarde.');

  // Injeta o HTML do flyer num div oculto na mesma pagina (sem iframe)
  // assim o html2canvas consegue capturar sem restricao cross-origin
  const wrap=document.createElement('div');
  wrap.style.cssText='position:fixed;left:-9999px;top:0;width:1080px;height:1080px;overflow:hidden;z-index:-1';
  wrap.innerHTML=html;
  document.body.appendChild(wrap);

  // Aguarda fontes e CSS carregarem
  ...
  try{
    ...
    // Fallback: abre o HTML numa janela para o usuario tirar print
    const blob=new Blob([html],{type:'text/html'});
    const url=URL.createObjectURL(blob);
    window.open(url,'_blank');
    toast('Abriu em nova aba — pressione Ctrl+Shift+S para salvar como imagem.');
  }finally{
